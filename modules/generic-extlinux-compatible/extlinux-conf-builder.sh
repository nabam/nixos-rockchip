...
addEntry() {
    ...
    echo "  LINUX /boot/nixos/$(basename $kernel)"
    echo "  INITRD /boot/nixos/$(basename $initrd)"
    ...
    if [ -d "$dtbDir" ]; then
        ...
        if [ -n "$dtbName" ]; then
            echo "  FDT /boot/nixos/$(basename $dtbs)/${dtbName}"
        else
            echo "  FDTDIR /boot/nixos/$(basename $dtbs)"
        fi
    else
        if [ -n "$dtbName" ]; then
