...
let
  ...
  cfg = blCfg.generic-extlinux-compatible-patched;
...
{
  options = {
    boot.loader.generic-extlinux-compatible-patched = {
      enable = mkOption {
        default = false;
        type = types.bool;
  ...
  in
    mkIf cfg.enable {
      ...
      boot.loader.generic-extlinux-compatible-patched.populateCmd = "${populateBuilder} ${builderArgs}";
    };
}
